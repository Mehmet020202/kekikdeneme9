...
class DramaIzle : MainAPI() {
    ...
    override val mainPage = mainPageOf(
        "${mainUrl}/son-eklenen" to "🔥 EN SON DRAMALAR",
        "${mainUrl}/son-bolumler" to "🔥 EN SON BÖLÜMLER", 
        "${mainUrl}/yeni-eklenen" to "🔥 YENİ EKLENEN DRAMALAR",
        "${mainUrl}/populer" to "🔥 POPÜLER DRAMALAR",
        "${mainUrl}/drama-listesi" to "Drama Listesi",
        "${mainUrl}/tam-bolum" to "Tam Bölüm",
        "${mainUrl}/film" to "Drama Filmleri"
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        try {
            ...
            val home = document.select("div.drama-card, div.drama-item, article.drama, .drama-list-item").mapNotNull { it.toSearchResult() }

            return newHomePageResponse(request.name, home)
        } catch (e: Exception) {
            Log.d("DRAMA", "Error loading main page: ${e.message}")
            return newHomePageResponse(request.name, emptyList())
        }
    }

    private fun Element.toSearchResult(): SearchResponse? {
        val title = this.selectFirst("h2, h3, .title, .drama-title")?.text()?.trim() ?: return null
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        try {
            ...
            return document.select("div.drama-card, div.drama-item, article.drama, .drama-list-item").mapNotNull { it.toSearchResult() }
        } catch (e: Exception) {
            Log.d("DRAMA", "Error in search: ${e.message}")
            return emptyList()
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        try {
            ...
            val title = document.selectFirst("h1, .drama-title")?.text()?.trim() ?: return null
            val poster = fixUrlNull(document.selectFirst("img.poster, .drama-poster img")?.attr("src")) ?:
                        fixUrlNull(document.selectFirst("img.poster, .drama-poster img")?.attr("data-src"))
            ...
            val recommendations = document.select(".similar-drama a, .recommendations a").mapNotNull {
            ...
            Log.d("DRAMA", "Error loading content: ${e.message}")
            return null
        }
    }
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            document.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                ...
                if (iframeSrc != null) {
                    Log.d("DRAMA", "Found iframe: $iframeSrc")
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("DRAMA", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("DRAMA", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("DRAMA", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            if (!foundLinks) {
                try {
                    ...
                    if (videoUrl != null && videoUrl.isNotEmpty()) {
                        Log.d("DRAMA", "Found AJAX video URL: $videoUrl")
                        callback.invoke(
                            newExtractorLink(
                                source = "AJAX Video",
                                name = "AJAX Video",
                    ...
                    Log.d("DRAMA", "Error with AJAX request: ${e.message}")
                }
            }
            ...
            if (!foundLinks) {
                document.select("script").forEach { script ->
                    try {
                        ...
                        for (pattern in patterns) {
                            ...
                            if (match != null) {
                                ...
                                if (videoUrl.isNotEmpty() && (videoUrl.contains("http") || videoUrl.contains(".m3u8") || videoUrl.contains(".mp4"))) {
                                    Log.d("DRAMA", "Found video URL in script: $videoUrl")
                                    callback.invoke(
                                        newExtractorLink(
                                            source = "Script Video",
                                            name = "Script Video",
                        ...
                        Log.d("DRAMA", "Error parsing script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("DRAMA", "Error in loadLinks: ${e.message}")
            return false
        }
    }
}
